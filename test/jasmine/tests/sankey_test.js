        ...
        it('should show the correct hover labels when hovertemplate is specified', function (done) {
            ...
            Plotly.newPlot(gd, mockCopy)
                ...
                .then(function () {
                    return Plotly.restyle(gd, {
                        'node.hovertemplate':
                            'hovertemplate<br>%{value}<br>%{value:0.2f}<br>%{customdata[0]}/%{customdata[1]}<extra>%{fullData.name}</extra>',
                        'link.hovertemplate':
                            'hovertemplate<br>source: %{source.label}<br>target: %{target.label}<br>size: %{value:0.0f}TWh<br>%{customdata[1]}<extra>%{fullData.name}</extra>'
                    });
                })
                .then(function () {
                    hoverNode('Solid');
